...
<body style="background-color: #eaeaea">
	<div class="container"  style="background-color: white; padding: 30px">
		<form action="inGServPro.do" method="post"
			...
			<table style="width: 700px">
					...
					<tr>
						...
						<td><input type="text" id="alarm" name="pickUpTime"
							style="min-width: 200px;" class="form-control">
						</td>
					</tr>
				</table>
			...
			<br>
				<span style="float:right; margin-bottom: 20px;">
					<input type="file" name="pickUpImg"><br>
			...
			<table id="TblAttach" class="table table-striped"
					...
					%>
					<tr>
						<td><%=num%><input type="hidden" name="gservOrder"
							value="<%=num%>"></td>
						<td><input type="text" name="introTitle" placeholder="ex) 해운대" class="form-control"></td>
						<td><textarea rows="5" cols="50" name="gservIntro"
								required="required" class="form-control"
				...
				<table style="width: 50%;">
					...
					<tr>
						<td><input type="text" name="gservPrice" id="gservPrice"
							placeholder="예) 10000" class="form-control"></td>
					</tr>
					<tr>
						<th>최소인원</th>
						<th>최대인원</th>
					...
					<tr>
						<td><input type="text" name="gservLang" id="gservLang"
							placeholder="예 ) 한국어, 영어" class="form-control"></td>
						<td><input type="number" name="gservLeadTime"
							id="gservLeadTime" placeholder="예 ) 1.5(=1시간 30분)"
							style="float: right;"
							class="form-control"></td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2">
							<input type="text" placeholder="#먹방 #꿀잼 #제주도" name="servTag" class="form-control">	
						</td>
					</tr>
				</table>
			<br>
...
1. 일정 중 점심: 1회 당 ￦10,000~￦25,000 정도 
2. 개인별 간식 및 기호품"  
					 ></textarea>
			</div>	
			...
			<textarea rows="10" cols="10" name="caution" required="required"
					  ...
					  onKeyUp="javascript:fnChkByte(this,'4000')"
					  placeholder="예) 
※ 교통비, 입장료 등은 현지 물가변동, 특별 전시와 같은 이유로 예고없이 인상될 수 있습니다. 
※ 연중 해운대에 인파가 몰릴 것으로 예상되는 날은 사전예약제(하이패스)로 진행합니다. 
이 경우 입장료에 예약비가 추가되며 예약자분들께 예약 티켓비 선입금 관련하여 개별 안내를 드립니다. 
예약 티켓은 정책상 구매 후 취소/환불이 불가능합니다."
					></textarea>
			</div>	
			<hr>
			<h5><b>노출여부</b></h5> 
			...
			<div style="margin-top: 50px; margin-bottom: 50px">
				<input type="submit" onclick="chk()" id="submit" value="등록"
						class="btn btn-info form-control"><input
						type="reset" value="취소" class="form-control">
			</div>
		</form>
	</div>
...
function addItem() {
      ...
      newTd.innerHTML= '<td><input type="text" class="form-control" name="introTitle" value=""><td>';
      newTd=newTr.insertCell(2);
      newTd.innerHTML= '<td><textarea rows="3" class="form-control" cols="50" name="gservIntro" required="required"></textarea></td>';
      ...
      newTd.innerHTML= '<td><input type="button" name="addServ" class="btn btn-default" onclick="addItem()" value="+"><input type="button" name="fixServ" class="btn btn-default" style="margin-left: 5px" value="X" onclick="delItem()"></td>';
...
<script type="text/javascript">
	...
	// 마커가 표시될 위치입니다 
	var markerPosition = new daum.maps.LatLng(37.55647223169107,
			126.94521202876426);
	// 마커를 생성합니다
